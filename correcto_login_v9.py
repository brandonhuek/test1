...
if not st.session_state.logueado:
    st.title("🔐 Acceso al Generador de Presupuestos")
...
for formato in formatos:
    precio = coste_bolsa_unitario[formato]
    cantidad = st.number_input(f"Bolsas de {formato} ({precio:.2f} €/ud)", key=f"input_{formato}", min_value=0, value=0, step=10)
...
minimo = minimos.get(modalidad, 0)
boton_desactivado = total_kg < minimo
if boton_desactivado:
    st.error(f"❌ No puedes descargar el presupuesto: la modalidad {modalidad} requiere mínimo {minimo} kg.")
if st.button(lang["descargar"], disabled=boton_desactivado):
    pdf = generar_pdf()
    try:
        import smtplib
        import ssl
...
""")
    ...
    st.download_button("Descargar PDF", key="descargar_pdf_button", data=pdf, file_name="presupuesto_cbd.pdf")

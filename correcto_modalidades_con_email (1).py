...
modalidad = st.selectbox(lang["modalidad"], ["Standard", "Premium", "Platino"])
    if modalidad == "Standard":
        st.info("**Standard:** En esta modalidad deberás previamente negociar un producto a Granel SOLO de nuestra Marca y el Empaquetado de productos será SOLO CBD, de las categorías hachises y flores. ESTA OPCION NO INCLUYE NINGUNA MOLECULA PREMIUM COMO: NANO9, NANO10, NANO XXX O MOLECULAS NUEVAS.\n\nCobramos precio de las bolsas + Mano de Obra")
        mostrar_imagenes = False
    elif modalidad == "Premium":
        st.info("**Premium:** En esta modalidad deberás enviarnos tus propias Bolsas con tu marca, también puedes negociar un producto a Granel de nuestra Marca o enviarnos tu propio producto a Granel.\n\nSolo Cobramos Mano de Obra.\n\nSi nos envías tu producto Pueden Haber gastos adicionales y/o mermas por mal estado del producto.")
        mostrar_imagenes = True
    elif modalidad == "Platino":
        st.info("**Platino:** En esta modalidad deberás previamente negociar un producto a Granel EXCLUSIVO de nuestra Marca, el Empaquetado de productos será: Nano9, Nano10, Nano XXX y nuevas moléculas de categoría hachises y flores.\n\nCobramos precio de las bolsas + Mano de Obra")
        mostrar_imagenes = False
...
recargo_eq = st.selectbox(lang["recargo_eq"], ["No", "Sí"])
    if mostrar_imagenes:
        st.markdown("#### Muestranos tu producto para empaquetar")
        imagenes = st.file_uploader("Sube hasta 4 imágenes", accept_multiple_files=True, type=["png", "jpg", "jpeg"])
    else:
        imagenes = []
...
# === ENVÍO DE EMAIL AUTOMÁTICO CON PDF ===
def enviar_presupuesto_email(nombre_cliente, email_cliente, pdf_bytes, logo_path):
    remitente = "crafters@innovationcbd.es"
    destinatarios = [email_cliente, "hola@innovationcbd.es"]
    asunto = f"Presupuesto personalizado - {nombre_cliente}"
    ...
    logo_cid = make_msgid()
    email_msg.set_content(f"""Hola {nombre_cliente},
...
""")
    ...
    email_msg.add_alternative(f"""<html>
  <body>
    <p>Hola {nombre_cliente},<br><br>
       Adjuntamos tu presupuesto personalizado.<br><br>
       <img src='cid:{logo_cid[1:-1]}' width='200'><br><br>
       Un saludo,<br>
       <b>Innovation Crafters</b>
    </p>
  </body>
</html>
""", subtype="html")
    ...
    # Insertar PDF
    email_msg.add_attachment(pdf_bytes, maintype="application", subtype="pdf", filename="presupuesto_cbd.pdf")

    # Insertar imagen del logo
    with open(logo_path, "rb") as img:
        email_msg.get_payload()[1].add_related(img.read(), "image", "png", cid=logo_cid)

    # Enviar email por SMTP
    contexto = ssl.create_default_context()
    try:
        with smtplib.SMTP_SSL("mail.innovationcbd.es", 465, context=contexto) as smtp:
            smtp.login("crafters@innovationcbd.es", "ArgentinA2013")
            smtp.send_message(email_msg)
        st.success("📧 Presupuesto enviado por email correctamente.")
    except Exception as e:
